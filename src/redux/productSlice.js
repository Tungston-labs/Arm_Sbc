...
// Admin Thunks
export const fetchProductsAdmin = createAsyncThunk("product/fetchProductsAdmin", async () => {
  const data = await listProductsAdmin();
  return data;
});

export const fetchProductAdmin = createAsyncThunk("product/fetchProductAdmin", async (productId) => {
...
export const fetchProductPublic = createAsyncThunk("product/fetchProductPublic", async (productId) => {
  const data = await getProductByIdPublic(productId);
  return data;
});

export const fetchRelatedProducts = createAsyncThunk("product/fetchRelatedProducts", async (productId) => {
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateExistingProduct.fulfilled, (state, action) => {
        const index = state.productsAdmin.findIndex((p) => p.id === action.payload.id);
